...
3. The host optionally turns on the **professional variant** and starts.
...
the same seat and the same hand. If somebody drops out mid-game, their lowest
card is played for them after 30 seconds so the table is never stuck.
...
test/         node:test suites for the rules and for the room protocol
```
...
`?server=https://your-worker.workers.dev` once, or paste the URL under
**Settings → Game server**. Both are remembered in `localStorage`.
...
Minimal, sharp and phone-first. Every corner is square, there are no gradients
and no shadows, and the only transition is a fade.
...
row you are about to take. The theme follows the system by default and can be
pinned under Settings.
